...
//--Caso contrário salvará na pasta selecionada.
	...
	While !(cAliasCT)->(Eof())
		...
		//@@ CUSTOMIZACAO SERRANA
		If TableInDic("SZK")
			SZK->(DbSetOrder(3))
			// Verifica se este CTE veio de uma importacao de outro sistema
			If SZK->(DbSeek(xFilial("SZK") + (cAliasCT)->DT6_DOC + (cAliasCT)->DT6_SERIE))

				xcDOc  := PADR(SZK->ZK_DOC,TAMSX3("DT6_DOC")[1])
				xcSerie:= PADR(SZK->ZK_SERIE,TAMSX3("DT6_SERIE")[1])
				xcCte  := xcSerie + xcDOc
				xdData := SZK->ZK_DTEMISS
				xcHora := SZK->ZK_HREMISS
				AADD(aXML,{SZK->ZK_PROTSEF,SZK->ZK_XML,xcCte,"","",xcHora,xdData})
			End If
		End if
		...
		If !Empty(aXML[nX][2])
			...
			//@@CUSTOMIZACAO SERRANA
			If AttIsMemberOf(oNfe,'_CTEPROC')
				oNfe := oNfe:_CTEPROC
			End if
		...
		If  oNfe:_CTE:_INFCTE:_IDE:_modal:TEXT == "06" // do tipo MultiModal
			...
			oDacte:Say(0651, 0210, "Informações Específicas do Transporte Multimodal de Cargas", oFont08N)
			oDacte:Line(0654, 0000, 0652, 0559)
			oDacte:Say(0661, 0003, "COTM da Empresa: " + cTmsCOTM	, oFont08)

			If (Type("oNFE:_CTE:_INFCTE:_INFCTENORM:_INFMODAL:_MULTIMODAL")) <> "U" //Existe Stru da MultiModal.
				cIndNegoci := oNFE:_CTE:_INFCTE:_INFCTENORM:_INFMODAL:_MULTIMODAL:_indNegociavel:TEXT

				oDacte:Box(0658, 0295, 0666, 0285)
				oDacte:Say(0664, 0300, "Negociável" , oFont08)

				oDacte:Box(0658, 0350, 0666, 0360)
				oDacte:Say(0664, 0368, "Não Negociável" , oFont08)
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ	
	...
	Else
		cQrCode := 'https://dfe-portal.svrs.rs.gov.br/cte/QRCode?chCTe=' + aCab[5] + '&tpAmb=' + AllTrim(STR((cAliasCT)->DT6_AMBIEN))
	EndIf
	If oDacte:nDevice == 2
		oDacte:QRCODE(200, 446, cQrCode, 115)	// Spool
	Else
		oDacte:QRCODE(200, 446, cQrCode, 115)	// PDF
	EndIf
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	...
	If	AllTrim((cAliasCT)->DT6_CHVCTG)<>''
		oDacte:Code128C(172.4,265,(cAliasCT)->DT6_CHVCTE, 24)
		oDacte:Code128C(208.2,265,(cAliasCT)->DT6_CHVCTG, 23)
	Else
		oDacte:Code128C(171.4,265,(cAliasCT)->DT6_CHVCTE, 24)
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	...
	ElseIf !Type("oNfe:_CTE:_INFCTE:_IMP:_ICMS:_ICMS60:_CST:TEXT") == "U"
		...
		//@@ Customizacao SERRANA
		//Conforme a norma tecnica Manual_CTe_v3_00 a tag _VCRED nao 'e obrigatoria
		//desta forma, impressao de DACTE de xml de outros sistema estava gerando errolog.
		If !Type("oNfe:_CTE:_INFCTE:_IMP:_ICMS:_ICMS60:_vCred:TEXT") == "U"
