...
While !(cAliasMDF)->(Eof())
	...
	If lXml
		...
		If TableInDic("SZN") 
			SZN->(DbSetOrder(2))
			//ZN_FILIAL+ZN_MDFE+ZN_SERIE		
			// Verifica se este MDFE veio de uma importacao de outro sistema
			If SZN->(DbSeek(xFilial("SZN") + (cAliasMDF)->DTX_MANIFE + cSerie))
				
				xcDOc  := PADR(SZN->ZN_MDFE,TAMSX3("DTX_MANIFE")[1])
				xcSerie:= PADR(SZN->ZN_SERIE,TAMSX3("DTX_SERMAN")[1])
				xcMdfe := xcSerie + xcDOc
				xdData := SZN->ZN_DTEMISS
				xcHora := SZN->ZN_HREMISS
				AADD(aXML,{SZN->ZN_PROTOCO,SZN->ZN_XML,xcMdfe,"","",xcHora,xdData})
			End If
		End if
	...
	//@@CUSTOMIZACAO SERRANA
	If AttIsMemberOf(oNfe,'_MDFEPROC') 
		oNfe := oNfe:_MDFEPROC
	End if
