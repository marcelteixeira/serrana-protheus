...
@type function
/*/
User Function SERR0002(cFornece,cLoja,cPrefixo,cNumero,aObs,oPrint)
...
Static Function Cabecalho(nLinha)
	...
	oPrinter:Say(nLinha + 0100,0555,"RECIBO DE PRODUÇÃO COOPERATIVISTA - RPC",oArial13N,,0)
	oPrinter:Say(nLinha + 0182,0555,Alltrim(SM0->M0_NOMECOM) ,oArial13N,,0) 
	oPrinter:Say(nLinha + 0275,0555,"CNPJ:" + TRANSFORM(SM0->M0_CGC,"@R 99.999.999/9999-99"),oArial13N,,0)
	oPrinter:Say(nLinha + 0275,1222,"Insc. Estadual: " + Alltrim(SM0->M0_INSC),oArial13N,,0)

	oPrinter:Box(nLinha + 0100,1934,0302,2331)
	oPrinter:Say(nLinha + 0222,1978,UPPER(MesExtenso(Month((cAliasSE2)->E2_EMISSAO))) + " / " + cvaltochar(Year((cAliasSE2)->E2_EMISSAO)),oArial13N,,0) 

	oPrinter:Box(nLinha + 0380,0105,0486,1934)
	oPrinter:Say(nLinha + 0421,0133,"Nome:",oArial13N,,0)
	oPrinter:Say(nLinha + 0464,0133,Alltrim((cAliasSE2)->A2_NOME),oArial11,,0)

	oPrinter:Box(nLinha + 0380,1934,0486,2333)
	oPrinter:Say(nLinha + 0421,1955,"Código",oArial13N,,0)
	oPrinter:Say(nLinha + 0464,1955,Alltrim((cAliasSE2)->E2_FORNECE),oArial11,,0)

	oPrinter:Box(nLinha + 0486,0105,0573,0884)
	oPrinter:Say(nLinha + 0522,0133,"Data Admissão:",oArial13N,,0)
	oPrinter:Say(nLinha + 0561,0133,DTOC((cAliasSE2)->RA_ADMISSA),oArial11,,0)

	oPrinter:Box(nLinha + 0486,0884,0573,1563)
	oPrinter:Say(nLinha + 0522,0910,"CPF:",oArial13N,,0)
	oPrinter:Say(nLinha + 0561,0910,Alltrim((cAliasSE2)->A2_CGC),oArial11,,0)

	oPrinter:Box(nLinha + 0486,1561,0573,2333)

	cPamCard := (cAliasSE2)->RA_YPAMCAR	

	If !Empty(cPamCard)
		oPrinter:Box(0486,1934,0573,2333)
		oPrinter:Say(nLinha + 0522,1955,"PamCard",oArial13N,,0)
		oPrinter:Say(nLinha + 0561,1955,cPamCard,oArial11,,0)
	End If

	oPrinter:Say(nLinha + 0522,1591,"Matrícula:",oArial13N,,0)
	oPrinter:Say(nLinha + 0561,1591,Alltrim((cAliasSE2)->RA_FICHA),oArial11,,0)
	...
	oPrinter:Line(nLinha +  590, 0105, 590, 2334) // Superior
	oPrinter:Line(nLinha +  640, 0105, 640, 2334) // Inferior
...
@type function
/*/
Static Function printPage(aObs)
	...
	For i:= 1 to Len(aVerbas)
		...
		If Mod( i, 46 ) == 0
			...
			oPrinter:Line( 590, 0105, nLinha, 0105) // Codigo
			oPrinter:Line( 590, 0300, nLinha, 0300) // Descricao
			oPrinter:Line( 590, 1047, nLinha, 1047) // Compentencia
			oPrinter:Line( 590, 1317, nLinha, 1317) // Referencia
			oPrinter:Line( 590, 1561, nLinha, 1561) // Vencimentos
			oPrinter:Line( 590, 1932, nLinha, 1932) // Descontos
			oPrinter:Line( 590, 2334, nLinha, 2334) // Descontos
