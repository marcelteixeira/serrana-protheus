...
±±³Desc.     ³ C lculo de comissäes off-line                                  ³±±
			...
			If lComVdBlq
				...
				If aBases[nBases][5] <> Nil .AND. !lBlqFor
					If ( (!lPago .Or. !cOrigem$"FINA440") .And. aBases[nBases][5] <> 0 )
						...
						SE3->E3_PORC    := Abs(Round(SE3->E3_COMIS/SE3->E3_BASE,nDecimalP+1)*100)	// % da ComissÆo ( na EmissÆo )
			...
			For nCntFor := 1 To nVend
				...
				If ( !Empty(cVendedor) .And. (cAliasSF4)->F4_DUPLIC == "S" )
					aadd(aSD2Vend,{ cVendedor,;
						...
						Iif(!Empty(cFieldPis), (cAliasSD2)-> D2_VALPIS,0) ,; // pis RETENÇÃO   
						Iif(!Empty(cFieldCof), (cAliasSD2)-> D2_VALCOF,0) }) // cofins retenção
						If cPaisLoc == "PTG"
...
Else 
	...
	If !Empty(cProcCom)
		...
		If Alltrim(SE2->E2_TIPO) = "DP" .and. !Empty(SE2->E2_BAIXA)
			Help(NIL, NIL, "BxNaoCanc.", NIL, STR0011+CRLF+CRLF+;// Baixa não pode ser cancelada. Título possui comissão paga.
		...
		ElseIF Alltrim(SE2->E2_TIPO) = "DP" .and. Empty(SE2->E2_BAIXA)
			IF lAutomato .Or. MsgYesNo(STR0011) //Existe Título de comissão gerado no Contas a Pagar para esta Baixa, deseja exclui-lo agora?..
		        ...
		        While (cAliasSE2)->(!EOF()) .and. Empty((cAliasSE2)->E2_BAIXA)
					...
					If Empty((cAliasSE2)->E2_BAIXA)
						MsgInfo( STR0011+CRLF+CRLF+;// Título relacionado abaixo referente à Comissão desta baixa cancelada/excluída foi excluído do Contas a Pagar.
																		STR0011 +' "'+Alltrim(cFilSE2)+'" '+CRLF+;
																		STR0011 +' "'+Alltrim(cPrefSE2)+'" '+CRLF+;
																		STR0011 +' "'+Alltrim(cNumSE2)+'" '+CRLF+;
																		STR0011 +' "'+Alltrim(cParcSE2)+'" '+CRLF+;
			...
			Else
				ALERT( STR0011+CRLF+CRLF+;// Título possui comissão gerada, se faz necessário excluir o Título de comissão relacionado abaixo no Contas a Pagar para que este Cancelamento/Exclusão seja Efetivo.
																STR0011 +' "'+Alltrim(cFilSE2)+'" '+CRLF+;
																STR0011 +' "'+Alltrim(cPrefSE2)+'" '+CRLF+;
																STR0011 +' "'+Alltrim(cNumSE2)+'" '+CRLF+;
																STR0011 +' "'+Alltrim(cParcSE2)+'" '+CRLF+;
